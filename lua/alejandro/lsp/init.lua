return {
    setup = function(ops)
        ops = vim.tbl_deep_extend("force", require "alejandro.lsp.default", ops)
        ...
        for server, options in pairs(ops.servers) do
            if options.enable == true then
                local ok, definition = pcall(require, "alejandro.lsp.servers." .. server)
        ...
        vim.api.nvim_create_user_command("AlphaLSP", function()
            vim.ui.select(servers, { prompt = "LSP Servers" }, function(server)
                ...
                local path = string.format("%s/lua/alejandro/lsp/servers/%s.lua", vim.fn.stdpath "config", server)
