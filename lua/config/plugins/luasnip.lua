...
function M.setup()
    ...
    ls.cleanup()
    for _, ft_path in ipairs(vim.api.nvim_get_runtime_file("lua/alejandro/snips/*.lua", true)) do
        local ft = vim.fn.fnamemodify(ft_path, ":t:r")
        ls.add_snippets(ft, make(loadfile(ft_path)()))
    end

    -- <c-k> para saltar a la siguiente posicion del snippet
    -- esto expandirá el elemento actual o saltará al siguiente elemento dentro del fragmento
    ...
    -- <c-j>  es para saltar a la posicion atras
